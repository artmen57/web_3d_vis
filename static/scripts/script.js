...
let uploadModel, login, register, logout, showLoginForm, showRegisterForm, 
    hideAuthForms, showMyModels, hideMyModels, loadModel, deleteModel,
    sendCommand, toggleAutoRotate, switchModeBySlider;

// Ждём полной загрузки DOM
document.addEventListener("DOMContentLoaded", function () {
  // Инициализация приложения
  init();
  // Привязка функций к глобальным переменным
    uploadModel = uploadModelFunc;
    login = loginFunc;
    register=registerFunc;
    hideAuthForms=hideAuthFormsFunc;
    logout=logoutFunc; 
    showLoginForm=showLoginFormFunc;
    showRegisterForm=showRegisterFormFunc; 
    hideAuthForms=hideAuthFormsFunc;
    showMyModels=showMyModelsFunc;
    hideMyModels=hideMyModelsFunc;
    loadModel=loadModelFunc;
    deleteModel=deleteModelFunc;
    sendCommand=sendCommandFunc;
    toggleAutoRotate=toggleAutoRotateFunc;
    switchModeBySlider=switchModeBySliderFunc;
    // и т.д.
});

function init() {
  // Весь существующий код переносится сюда
  //import { OrbitControls } from "https://unpkg.com/three@0.112/examples/jsm/controls/OrbitControls.js";
...
const height = Math.round(width * 0.75); // Соотношение 4:3
...
${
  model.thumbnail
    ? `<img src="data:image/jpeg;base64,${model.thumbnail}" class="model-thumbnail">`
    : ""
}
<span>${model.name} (${model.vertex_count} vertices)</span>
    </div>
    <div>
<button onclick="loadModel(${model.id})">Загруить</button>
...
`;
  ...
  document.getElementById("saveToDb").addEventListener("change", function (e) {
    document.getElementById("modelNameInput").style.display = e.target.checked
      ? "block"
      : "none";
  });
 ...
 */
  ...
  function connect() {
    ...
    ws.onmessage = function (event) {
      ...
      if (data.type === "session_created") {
        ...
        sessionInfo.innerHTML = `Session ID: <span style="color: #0088ff; font-family: monospace;">${sessionId.substring(
          0,
          8
        )}</span>`;
  ...
  function toggleAutoRotate() {
    ...
    autoRotateBtnEl.textContent = autoRotateEnabled
      ? "⏸️ Stop Rotation"
      : "🔄 Auto Rotate";
    autoRotateBtnEl.style.background = autoRotateEnabled
      ? "#cc6600"
      : "#0066cc";
  }
  ...
  async function uploadModel() {
    ...
    if (!sessionId) {
      alert(
        "No active session. Please wait for connection or refresh the page."
      );
      return;
    }
    ...
    if (saveToDb) {
      const modelName =
        document.getElementById("modelNameInput").value || file.name;
    ...
    try {
      ...
      if (response.ok) {
        ...
        uploadStatusEl.textContent = `✅ Successfully loaded: ${result.model_name}`;
        ...
        const error = await response.json();
        uploadStatusEl.textContent = `❌ Error: ${error.detail}`;
    ...
    uploadBtnEl.disabled = false;
    uploadBtnEl.textContent = "📁 Upload OBJ File";
  }
 ...
 */
  ...
  function initThreeJS() {
    ...
    threeControls.enableDamping = true; // плавность вращения
    threeControls.dampingFactor = 0.05;
    threeControls.enableZoom = true; // колесо/щипок
    threeControls.autoRotate = false; // автоворот по кнопке
    threeControls.autoRotateSpeed = 1.0;
    //threeRenderer.domElement.style.touchAction = 'none';

    // Обновить параметры камеры для мобильных
    ...
    for (const line of lines) {
      ...
      if (parts[0] === "v") {
        ...
        if (faceIndices.length === 3) {
          faces.push(
            new THREE.Face3(faceIndices[0], faceIndices[1], faceIndices[2])
          );
          ...
          faces.push(
            new THREE.Face3(faceIndices[0], faceIndices[2], faceIndices[3])
          );
        }
      }
    }
